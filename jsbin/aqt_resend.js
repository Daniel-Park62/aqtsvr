...
async function dataHandle(rdata, qstream) {
  ...
  const req = http.request(options, function (res) {
    ...
    res.on('end', async function () {
      ...
      await con.query("UPDATE ttcppacket SET \
        ...
        , [rDatas, Buffer.from(new_shead), stime.toSqlfmt(stimem), rtime.toSqlfmt(rtimem), svctime
          , res.statusCode, resHs, rsz, rdata.pkey])
        .then(console.log("** update ok:", rdata.pkey, uri))
        .catch(err => console.error('update error:', rdata.pkey, err))
  ...
  req.on('error', async (e) => {
    ...
    await con.query("UPDATE ttcppacket SET \
      ...
      .then(console.log("** error update ok:", rdata.pkey, uri))
      .catch(err => console.error('update error:', rdata.pkey, err))
      .finally(qstream.resume());
