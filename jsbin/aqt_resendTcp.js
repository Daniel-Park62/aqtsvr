...
setInterval(async () => {
    ...
    qstream.on("data", async row => {
        ...
        await con.query("SELECT t.pkey,o_stime, if( ifnull(m.thost,IFNULL(c.thost,''))>'',ifnull(m.thost,c.thost) ,dstip) dstip, if(ifnull(m.tport,IFNULL(c.tport,0))>0, ifnull(m.tport,c.tport), dstport) dstport,uri,method,sdata, rlen " + 
