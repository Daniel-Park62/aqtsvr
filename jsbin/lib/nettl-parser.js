...
var GLOBAL_HEADER_LENGTH = 128; //bytes
var PACKET_HEADER_LENGTH = 68; //bytes
...
function parseGlobalHeader() {
  ...
  if (buffer.length >= GLOBAL_HEADER_LENGTH) {
    var msg;
    var magicNumber = buffer.toString('hex', 0, 12);

    // determine pcap endianness
    if (magicNumber == "545200640000000000000080") {
      this.endianness = "BE";
    } else {
      this.errored = true;
      this.stream.pause();
      msg = util.format('not hpux hettl file: %s', magicNumber);
    ...
    var header = {
      magicNumber: 9,
      majorVersion: 0, // buffer['readUInt32' + this.endianness](8, true),
      minorVersion: 0,
...
function parsePacketHeader() {
  var buffer = this.buffer;
  if (buffer.length < PACKET_HEADER_LENGTH) return false ;
  let ph_length = buffer['readUInt16' + this.endianness](0, true) ;
  
  if (buffer.length >= ph_length) {
    var header = {
      originalLength: buffer['readUInt32' + this.endianness](32, true),
      // capturedLength: buffer['readUInt32' + this.endianness](4, true),
      packetLength: buffer['readUInt32' + this.endianness](32, true),
      capturedLength: buffer['readUInt32' + this.endianness](36, true)  ,
      timestampSeconds: buffer['readUInt32' + this.endianness](40, true),
      timestampMicroseconds: buffer['readUInt32' + this.endianness](44, true)
    };

    this.currentPacketHeader = header;
    this.emit('packetHeader', header);
    this.buffer = buffer.slice(ph_length);
