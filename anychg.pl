...
sub Usage {
  die <<END
  $0 [-i 검색값][-f 검색값파일] [-c dir] 디렉토리 검색파일
  -c dir : Dir to save the changed source
...
my (%ipNhost, @keysv) ;
if ( defined($opts{i})  ) {
	%ipNhost= map{s/^\s+// ; my($a,$b) = split(/\s+/,$_); if ($a =~ /\S+/) {push (@keysv, $a) ; $a => $b} } split("@@",$opts{i});
    
} elsif ( defined($opts{f}) ) {
	open (my $FX,"<",$opts{f}) || print STDERR "$? $opts{f}\n" && return ;
	binmode $FX ;
	%ipNhost=map{s/^\s+// ;my($a,$b) = split(/\s+/,$_); if ($a =~ /\S+/) {push (@keysv, $a) ; $a => $b} } <$FX> ;
} else {
	%ipNhost=map{s/^\s+// ;my($a,$b) = split(/\s+/,$_); if ($a =~ /\S+/) {push (@keysv, $a) ; $a => $b}  } <DATA> ;
} 
#my $reg = qr!\b(10|172|192)(?:\.\d{1,3}){3}\b!x ;
my $reg = join("|",keys %ipNhost) ;
